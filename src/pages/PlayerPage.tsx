...
const IcRewind = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
    <path d="M12 5C7.86 5 4.5 8.36 4.5 12.5S7.86 20 12 20s7.5-3.36 7.5-7.5" stroke="white" strokeWidth="2" strokeLinecap="round" />
    <polyline points="7,2.5 7,7 11.5,7" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <text x="12" y="15" fontSize="5.5" fill="white" fontFamily="system-ui,sans-serif" fontWeight="800" textAnchor="middle">10</text>
  </svg>
);
const IcForward = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
    <path d="M12 5c4.14 0 7.5 3.36 7.5 7.5S16.14 20 12 20 4.5 16.64 4.5 12.5" stroke="white" strokeWidth="2" strokeLinecap="round" />
    <polyline points="17,2.5 17,7 12.5,7" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <text x="12" y="15" fontSize="5.5" fill="white" fontFamily="system-ui,sans-serif" fontWeight="800" textAnchor="middle">10</text>
  </svg>
);
const IcLock = () => (
...
const IcFilm = () => (
  <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="1.5" strokeLinecap="round">
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (gestureTypeRef.current === 'v' && isNativeSource) {
      ...
      if (isRightSide && videoRef.current) {
        ...
        setBrightness(newBr);
        triggerHUD('brightness', newBr > 100 ? 100 : newBr);
      }
      touchStartRef.current = { x: touch.clientX, y: touch.clientY, time: Date.now() };
    }
  };
  ...
  const handleTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (gestureTypeRef.current === 'swipedown') {
      if (containerY > 90) {
        ...
        setContainerY(window.innerHeight);
        ...
        {!showSplash && isNativeSource && (
          <>
            {activeSource === 'archive' && archiveLoading && (
              <div style={{ ...fill, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14 }}>
                <div style={{
                  width: 28, height: 28,
            ...
            {activeSource === 'archive' && !archiveLoading && !archiveUrl && (
              <div style={{ ...fill, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                <IcFilm />
            ...
            {srcUrl && (
              <video
                ...
                key={srcUrl}
                src={srcUrl}
                style={{ ...fill, objectFit: objectFit, filter: brightness !== 100 ? `brightness(${brightness}%)` : 'none' }}
                autoPlay
                playsInline
              />
            )}
          ...
          >
            ...
            {isNativeSource && (
              <>
                ...
                {/* Row 2: Speed, Fit, Fullscreen, Lock */}
            ...
            <div style={{
              ...
              <span style={{
                ...
                flexShrink: 0, marginRight: 2, letterSpacing: '0.3px',
              }}>
                Source
              </span>
              {allSources.map(src => (
          ...
          >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  {['480p', '720p', '1080p', '4K'].map(q => (
                    <option key={q} value={q} style={{ background: '#0c0916' }}>{q}</option>
                  ))}
                </select>
              </div>
              <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  padding: '10px 26px',
                  background: uploadSaving || !uploadUrl.trim() ? 'rgba(124,58,237,0.38)' : '#7c3aed',
                  border: 'none', borderRadius: 11,
                  color: '#fff', fontSize: 14, fontWeight: 700,
                  cursor: uploadSaving || !uploadUrl.trim() ? 'not-allowed' : 'pointer',
                  outline: 'none',
