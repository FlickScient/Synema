...
export function DesktopTopBar() {
  ...
  const getChipActive = (chip: typeof FILTER_CHIPS[0]) => {
    ...
    if (chip.label === 'All') return location.pathname === '/';
    return location.pathname === chip.path && !currentGenre;
  };

  const handleChipClick = (chip: typeof FILTER_CHIPS[0]) => {
    if (chip.genre) {
